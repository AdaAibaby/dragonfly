  ...
  if (auto check_result = defrag_state_.CheckRequired(); check_result == NotSkipped) {
    ...
    if (scan_incomplete) {
      ...
      if (max_burst_cycles > 0 && defrag_state_.consecutive_burst_cycles >= max_burst_cycles) {
        // Burst budget spent (measured in real elapsed time, not invocation count): force a
        // cooldown before we're allowed to spin again, capping the achievable duty cycle instead
        // of running flat-out until the whole pass completes.
