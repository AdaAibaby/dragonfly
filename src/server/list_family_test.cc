...
TEST_F(ListFamilyTest, LMPopCountLargerThanList) {
  ...
  // Count above uint32 range must pop the whole list, not truncate.
  auto resp = Run({"lmpop", "1", "a", "LEFT", "COUNT", "4294967297"});
  EXPECT_THAT(resp, RespArray(ElementsAre("a", RespArray(ElementsAre("a1", "a2", "a3")))));
  EXPECT_THAT(Run({"exists", "a"}), IntArg(0));

  Run({"rpush", "b", "b1", "b2"});
  resp = Run({"lmpop", "1", "b", "LEFT", "COUNT", "9223372036854775807"});
  EXPECT_THAT(resp, RespArray(ElementsAre("b", RespArray(ElementsAre("b1", "b2")))));
